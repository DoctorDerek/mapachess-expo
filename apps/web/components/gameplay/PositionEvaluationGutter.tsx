...
export default function PositionEvaluationGutter({
  ...
  return (
    <div
      ...
      className="relative h-10 w-full overflow-hidden rounded-[0.65rem_0.15rem_0.65rem_0.15rem] border-[3px] border-[var(--mapachito-ink)] bg-[var(--mapachito-ink)] shadow-[0.25rem_0.25rem_0_var(--mapachito-coral),0.45rem_0.45rem_0_var(--mapachito-sun)] xl:h-full xl:w-[clamp(2rem,2.75vw,3rem)]"
    ...
    >
      <div
        aria-hidden="true"
        className={`absolute inset-y-0 left-0 w-[var(--white-share)] bg-[var(--mapachito-cream)] transition-[width,height] duration-300 motion-reduce:transition-none xl:inset-x-0 xl:inset-y-auto xl:h-[var(--white-share)] xl:w-auto ${orientation === "white" ? "xl:bottom-0" : "xl:top-0"}`}
      />
      <span
        aria-hidden="true"
